...
def all_documents_file_name(weaviate_client_v4: weaviate.Client) -> list[str]:
    """Get the `file_name` of all `Document` objects stored in Weaviate"""
    collection = weaviate_client_v4.collections.get("Document")
    for item in collection.iterator():
        logging.info(item)
    #'uuid':item.uuid,
    return [{'uuid':str(item.uuid),'name':item.properties['file_name']} for item in collection.iterator()]


def delete_document_by_id(weaviate_client: weaviate.Client, document_id: str) -> bool:
    """Delete a `Document` based on its Weaviate UUID."""
    try:
        weaviate_client.data_object.delete(uuid=document_id, class_name="Document")
        return True
...
def _summarize_text__openai(prompt: str, summarize_model_name: str) -> str:
    ...
    response = client.chat.completions.create(model=summarize_model_name, messages=[{"role": "user", "content": prompt}], temperature=0)
