...
def delete_documents(document_ids: list, server_url: str = SERVER_URL):
    """Send PUT request to FastAPI /stored_documents endpoint"""
    st.write(document_ids)
    response = requests.put(f"{server_url}/delete_documents", json={"ids": document_ids})
    return response

def get_all_documents_file_name(server_url: str = SERVER_URL):
