...
def weaviate_client_v4(vector_db_url: str, openai_api_key: str) -> weaviate.Client:
    """Instantiate Weaviate client for the local instance based on the url"""
    wv_connect = urlparse(vector_db_url)
    client = weaviate.connect_to_local(
        host = wv_connect.hostname,
        port = wv_connect.port,
        grpc_port = 50051, # hardcoded
...
def get_schema(weaviate_client: weaviate.Client) -> dict:
